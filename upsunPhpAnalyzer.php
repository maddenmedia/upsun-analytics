...
<?php
/*
 * Upsun PHP access log analyzer — derivative of platformsh-analytics.
...
if (!isset($_SERVER['argv'][1]) || !file_exists($_SERVER['argv'][1])) {
	exec('upsun projects --format=csv --no-header --count=0', $output);
...
if(!isset($_SERVER['argv'][1]) || !file_exists($_SERVER['argv'][1])){
    ...
    exec('upsun log --lines=' . escapeshellarg($numberOfLines) . ' --project=' . escapeshellarg($selectedProject[0]) . ' --environment=' . escapeshellarg($selectedEnvironment[0]) . ' php.access', $logData);
    echo 'Done' . "\n";
} else {
...
<html>
    ...
    <body>
            ...
            <div class="row mt-4">
                <div class="col-md-12">
                    ...
                    <p>
                        ...
                        // Illustrative memory tiers for rough comparison (adjust to your Upsun sizing).
                        $referenceResourceTiers = [
                            [
                                'memory' => .5,
                                'pageViewsMin' => 100000,
                                'label' => 'Small'
                        ...
                        ?>
						<strong>Reference resource tiers:</strong> <br>
						(% of page views / avg. % of total memory/minute / % of peak memory/minute)
						<ul>
						<?php
                        foreach ($referenceResourceTiers as $tier) {
                        	?>
							<li>
								<?php echo htmlspecialchars($tier['label']) ?>:
                                <?php echo round(
                                    ($requestsPerMonth/$tier['pageViewsMin'])*100
                                ) ?>% /
								<?php echo round(
									((($averageMemoryUsage*$requestsPerMinute)/1048576)/$tier['memory']), 2
								) ?>% /
                                <?php echo round(
                                    ((($peakMemoryUsage*$requestsPerMinute)/1048576)/$tier['memory']), 2
                                ) ?>%
							</li>
							<?php
						}
        ...
        <div class="container">
            ...
            // Bucket time slots (nested for loops must not double-increment the outer index).
            $xLabels = [];
            for ($bucketStart = 0; $bucketStart < count($dateTimeSlots); $bucketStart += $hoursPerSlot) {
                $xLabels[] = $dateTimeSlots[$bucketStart];
            }
            ...
            foreach (array_keys($responseCodesByTime) as $responseCode) {
                ...
                for ($bucketStart = 0; $bucketStart < count($dateTimeSlots); $bucketStart += $hoursPerSlot) {
                    $endI = min($bucketStart + $hoursPerSlot, count($dateTimeSlots));
                    $combinedSum = 0;
                    $j = 0;
                    for ($k = $bucketStart; $k < $endI; $k++) {
                        $j++;
                        $timeSlot = $dateTimeSlots[$k];
                        if (isset($responseCodesByTime[$responseCode][$timeSlot])) {
                            $combinedSum += round(
                                ($responseCodesByTime[$responseCode][$timeSlot] / $totalRequestsPerTimeSlot[$timeSlot]) * 100,
                                1
                            );
                        }
                    }
                    $responseCodeDataSets[$responseCode][] = $j > 0 ? $combinedSum / $j : 0;
                }
            }
						        ...
						        yAxes: [
						        	{
										id: 'requestsAxis',
										ticks: {
											max: <?php echo !empty($totalRequestsDataSets) ? max($totalRequestsDataSets) : 1 ?>,
											min: 0
										},
										display: false
									},
									{
								        id: 'memoryAxis',
								        ticks: {
									        max: <?php echo !empty($averageMemoryDataSets) ? max($averageMemoryDataSets) : 1 ?>,
									        min: 0
									        },
										display: false
							        },
							        {
								        id: 'cpuAxis',
								        ticks: {
									        max: <?php echo !empty($averageCpuDataSets) ? max($averageCpuDataSets) : 1 ?>,
									        min: 0
									        },
								        display: false
							        },
							        {
								        ...
								        ticks: {
									        max: <?php echo !empty($averageExecutionTimeDataSets) ? max(1, max($averageExecutionTimeDataSets)) : 1 ?>,
									        min: 0
									        },
								        display: false,
										type: 'logarithmic'
            ...
            <?php
                ...
                $topRequestsByMemory = [];
                foreach ($lineData as $line) {
                    $topRequestsByMemory[] = $line;
                    usort($topRequestsByMemory, static function ($a, $b) {
                        return $b['peakMemory'] <=> $a['peakMemory'];
                    });
                    if (count($topRequestsByMemory) > 10) {
                        array_pop($topRequestsByMemory);
                    }
                }

                $topRequestsByCpu = [];
                foreach ($lineData as $line) {
                    $topRequestsByCpu[] = $line;
                    usort($topRequestsByCpu, static function ($a, $b) {
                        return $b['cpuPercentage'] <=> $a['cpuPercentage'];
                    });
                    if (count($topRequestsByCpu) > 10) {
                        array_pop($topRequestsByCpu);
                    }
                }

                $topRequestsByExecutionTime = [];
                foreach ($lineData as $line) {
                    $topRequestsByExecutionTime[] = $line;
                    usort($topRequestsByExecutionTime, static function ($a, $b) {
                        return $b['executionTime'] <=> $a['executionTime'];
                    });
                    if (count($topRequestsByExecutionTime) > 10) {
                        array_pop($topRequestsByExecutionTime);
                    }
                }
            ?>
